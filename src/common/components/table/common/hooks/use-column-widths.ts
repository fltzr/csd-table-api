...
import type { TableColumnDefinition, TableColumnWidth } from '../../../../types/table';
 ...
 */
export const useColumnWidths = <T>({ localstorageKey, columnDefinitions }: UseColumnWidthsParams<T>) => {
  const [widths, saveWidths] = useLocalStorage<TableColumnWidth[]>({
    localstorageKey,
    initialValue: [] as TableColumnWidth[],
  });

  const handleWidthChange = (event: NonCancelableCustomEvent<TableProps.ColumnWidthsChangeDetail>) => {
    saveWidths(
